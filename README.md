...
14. #### Next, replace the actual related pages with:
```php
<ul>
  <?php
    if($post->post_parent)
...
18. #### Then add the following CSS to the style.css file:
...
20. #### Now add the following into your 990 media query in your style.css file.
...
26. #### Now, let’s create the widgetized area in the sidebar. This will allow us to place in a custom “Blog” sidebar. Let’s first register this area by adding the following into the functions.php file:
```php 
...
28. #### Next, find where you would like to add this area, generally above the Related Pages area:
```php 
...
30. #### Next, add the following to style the buttons:
```css
.moretag
	{
...
32. #### Let’s create the page that will display the search results. First, let’s register the search bar, add the following to the functions.php file:
```php
...
33. #### Now, let’s set the search results. Place the following into the functions.php file:
```php
...
34. #### Now, place the following code where you would want to add the search bar (if it’s not in the mockup, generally I will place it into the footer):
```php
...
35. #### Now, let’s create the search results page. Create a file called search.php. Again, copy the contents of page.php and replace the loop with the following:
```php
<!-- SEARCH PAGE -->
